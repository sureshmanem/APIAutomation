...
public class APIVerification extends ExtentReportListener{
	...
	public static void responseCodeValidation(Response response, int statusCode) {
		try {
			...
			test.log(LogStatus.FAIL, "Exception is " + e.fillInStackTrace());
			test.log(LogStatus.FAIL, "Expected status is : " + statusCode + ", but getting " + response.getStatusCode());
		}catch(Exception e) {
			test.log(LogStatus.FAIL, "Exception is " + e.fillInStackTrace());
		}
	}
	
	public static void responseKeyValidationFromArray(Response response, String key, String strName) {
		try {
			//Validating the Response
			JSONArray array = new JSONArray(response.getBody().asString());
			for(int i=0;i<array.length();i++) {
				JSONObject obj = array.getJSONObject(i);
				test.log(LogStatus.PASS, "Validated " + strName + " and values are " + obj.get(key));
			}
		}catch(Exception e) {
			test.log(LogStatus.FAIL, "Exception is " + e.fillInStackTrace());
		}
